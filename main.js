...
function createCarShape(carData) {
  const transform = `rotate(${carData.rotation + 270},100,100)`;
  const svg = carData.isLoco
  ? `<polygon points="50,90 50,110 140,110 150,100 140,90" transform="${transform}" fill="goldenrod" fill-opacity="50%" stroke="goldenrod" stroke-width="1%"/>`
  : `<rect x="50" y="90" width="100" height="20" transform="${transform}" fill="magenta" fill-opacity="50%" stroke="magenta" stroke-width="1%"/>`;
  return svg;
}
